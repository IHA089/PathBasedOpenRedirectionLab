...
lab_name = "PathBasedOpenRedirectionLab"
...
@PathBasedOpenRedirection.route('/redirect/<path:destination>')
def secure_redirect(destination):
    if destination.startswith("iha089-labs.in"):
        return redirect(f"https://{destination}")
    return "Invalid redirect URL.", 400

@PathBasedOpenRedirection.route('/')
def home():
    if not check_cookies():
        session.clear()
    return render_template('index.html', user=session.get('user'))

@PathBasedOpenRedirection.route('/index.html')
def index_html():
...
@PathBasedOpenRedirection.route('/login.html')
def login_html():
    if not check_cookies():
        session.clear()
    return render_template('login.html')

@PathBasedOpenRedirection.route('/join.html')
def join_html():
    if not check_cookies():
        session.clear()
    return render_template('join.html')

@PathBasedOpenRedirection.route('/forgot-password.html')
...
@PathBasedOpenRedirection.route('/acceptable.html')
def acceptable_html():
    if not check_cookies():
        session.clear()
...
@PathBasedOpenRedirection.route('/privacy.html')
def privacy_html():
    if not check_cookies():
        session.clear()
...
@PathBasedOpenRedirection.route('/confirm', methods=['POST'])
def confirm():
...
@PathBasedOpenRedirection.route('/resend', methods=['POST'])
def resend():
    username = request.form.get('username')
    password = request.form.get('password')
    hash_password=hashlib.md5(password.encode()).hexdigest()
...
@PathBasedOpenRedirection.route('/login', methods=['GET', 'POST'])
def login():
...
@PathBasedOpenRedirection.route('/resetpassword', methods=['POST'])
def resetpassword():
    password=request.form.get('password')
...
@PathBasedOpenRedirection.route('/join', methods=['GET', 'POST'])
def join():
    if not check_cookies():
        session.clear()
    if 'user' in session:
...
@PathBasedOpenRedirection.route('/forgot', methods=['POST'])
def forgot():
    try:
...
@PathBasedOpenRedirection.route('/dashboard')
@PathBasedOpenRedirection.route("/dashboard.html")
@login_required
def dashboard():
    if not check_cookies():
        session.clear()
...
@PathBasedOpenRedirection.route('/logout.html')
def logout():
    session.clear() 
...
@PathBasedOpenRedirection.route('/profile')
@PathBasedOpenRedirection.route('/profile.html')
@login_required
def profile():
    if not check_cookies():
        session.clear()
...
@PathBasedOpenRedirection.route('/reset_password', methods=['POST'])
def reset_password():
    username = request.form.get('username')
...
@PathBasedOpenRedirection.after_request
